...
import crnn_train.utils as utils
import crnn_train.mydataset as mydataset
import crnn_train.crnn as crnn
import crnn_train.config as config
from crnn_train.online_test import val_model
config.imgW = 800
config.alphabet = config.alphabet_v2
config.nclass = len(config.alphabet) + 1
config.saved_model_prefix = 'CRNN-1011'
config.train_infofile = ['information.txt']
config.val_infofile = 'information.txt'
config.keep_ratio = True
config.use_log = True
config.pretrained_model = './CRNN-1010.pth'
config.batchSize = 80
config.workers = 10
config.adam = True
# config.lr = 0.00003
...
def val(net, dataset, criterion, loss, max_iter=100):
    print('Start val')
    ...
    if accuracy > best_acc:
        best_acc = accuracy
        torch.save(crnn.state_dict(), '{}/{}_{}_{}_{}.pth'.format(config.saved_model_dir, config.saved_model_prefix, epoch,
                                                               int(best_acc * 1000), loss))
...
if __name__ == '__main__':
    ...
    if config.pretrained_model != '' and os.path.exists(config.pretrained_model):
        ...
        crnn.load_state_dict(torch.load(config.pretrained_model, map_location=torch.device('cpu')))
    else:
